...
| Enumeration (`sync_catalog.py`) | Done. **18,132 videos** cached in `data/catalog/`, **9,607 surviving talks** — 10,064 before the 2023 year floor, see *The pre-2023 cut* in `HISTORY.md` — plus **224 InfoQ-only presentations**, **9,831 in the corpus**. 9,249 of them enumerated from YouTube, 358 seeded — see *The WeAreDevelopers import* in `HISTORY.md`. It was 9,797 before the 2026-09-14 refresh, which added 35 talks and dropped one — the InfoQ-only *Understanding Progressive Collapse* became the YouTube record `ECUd5FuK5q4`, which is why InfoQ-only went 225 → 224; see *The refresh of 2026-09-14* in `HISTORY.md`. The jump from 9,048 on 2026-09-06 is mostly 722 already-cached videos whose descriptions arrived that day and now pass the AI filter, see *The refresh of 2026-09-06* in `HISTORY.md`. |
| Speakers | **5,673 of 9,831 (58%)** after the 2026-09-14 refresh; 5,653 of 9,797 after 2026-09-06; 4,982 of 9,048 (55%) since 2026-09-02, up from 3,447 (38%): the bold-Unicode `Speakers:` heading Microsoft's channels use, the `A & B, Company` and `by A and B` title shapes. See *Review of 2026-09-02* in `HISTORY.md`, section D, and `test_speakers.py`. |
| Enrichment (`enrich.py`) | Done for the whole catalog. **9,598 videos via the Data API** in one run (see *What the collection runs actually got* in `HISTORY.md`), then **4,715 more on 2026-09-06** with `--all --include-unknown-year` (82 unavailable, ~95 Data API units) — every cached video that can be enriched now is. See *The refresh of 2026-09-06* in `HISTORY.md`. |
| Per-talk markdown | Done. 9,831 files, regenerated from `talks.json` on every sync. |
| Search indexes (`build_index.py`) | Done. SQLite FTS5 + sharded browser index. Since 2026-09-02 the passages overlap at a half-passage stride and `talks.db` carries `PRAGMA user_version` (`atu.DB_SCHEMA_VERSION`, now 6) — 423.5 MiB after the 2026-09-14 refresh (420.7 after 2026-09-06, was 379), 48 s to build, and `atu.connect()` rebuilds it by itself when it is stale. The browser index is **keyed on Porter stems** (`atu.stem()`, memoised; the same function in JavaScript inside `index.html`, `test_stem.py` diffs them over the corpus vocabulary) and carries the **whole description's postings** per stem (`d`) plus the metadata document frequency (`m`): 49.5 MiB in 723 shards, 59,042 stems, 41,514 of them in descriptions; `search-meta.json` 5.62 MiB, 94% of its trigger, now display-only — and only because `META_DESC_CHARS` in `build_index.py` was halved 300 → 150 on 2026-09-06 when the file reached 6.92 MiB. That was the first halving; per `specs/search-browser.md` the next crossing should move descriptions out of the up-front payload rather than halve again. `tindex/_manifest.json` carries the **synonym groups** (`atu.SYNONYMS`) both rankers expand, and a record has `lg` when its transcript is not English. Byte-identical run to run. See *Review of 2026-09-02* and *Search enrichment* in `HISTORY.md`. |
| Topics | Done, 2026-09-02. **7,950 of 9,831 talks carry at least one of fifteen per-talk topics**, 1,881 none (76 of those have no description) — 7,922 of 9,797 before the 2026-09-14 refresh, 7,162 of 9,048 before 2026-09-06. Keyword rules in `atu.TOPICS` over title, tags and description, never the transcript; channel boilerplate stripped per conference first. `--topic` in the CLI, a Topic select and chips in the browser. See *The topic facet — built* in `HISTORY.md` and *What a talk is about* in `ARCHITECTURE.md`; the open item is a human read of twenty titles per topic (`TODO.md`). |
...
| Other offline tests | `test_query.py` (75: the parser, column filters, exclusions, synonyms, id resolution, `--topic`/`--category` resolution, the fusion helpers), `test_topics.py` (509, the topic rules, one check per boundary), `test_semantic.py` (60 without the layer, on the standard library: staleness, reciprocal-rank fusion, the pool mapping; its end-to-end block skips when the layer is absent), `test_infoq.py` (54, the fold-in and, since 2026-09-06, `merge_source`'s null-channel and recency-cap rules), `test_speakers.py` (18, every speaker shape and the false positives), `test_stem.py` (5: both stemmers over all 104,061 corpus tokens; skips its node half if node is missing). All green. |
| Fetcher tests (`tools/test_fetch_transcripts.py`) | Done. **157 offline checks** over the pool, the routes, the year selection, the four failure classes, the off-IP lease rule and the caption-language selection. |
...
| Transcripts | **3,437 of 9,831** over 35 conferences — 3,206 with exact timings (3,210 files, four held back from the index as ASR failures) and 231 estimated, from infoq.com. Was 3,409 of 9,797 before the 2026-09-14 refresh and 3,174 of 9,048 before 2026-09-06; before InfoQ 2,946 of 8,822, all exact. ai-engineer 571, wearedevelopers 440, qcon-infoq 339, pydata 244, microsoft-build 187, ndc 177, berkeley-agentic-ai-summit 159, kubecon 151, ai-devcon-tessl 140, black-hat 117, ai-council 94, mcp-dev-summit 84. **The 2026 scope is complete**: of its 3,242 talks, 3,199 are indexed with a transcript (3,203 files) and 39 are in `_misses.json` with no captions or a members-only 403, so nothing is pending — the 32 the 2026-09-14 refresh brought in were fetched the same day, and the 241 of 2026-09-06 before them; see *The refresh of 2026-09-14* in `HISTORY.md`. `I1GvlW1H4WI` entered the scope when the year-from-title fallback was fixed and was fetched for one credit — see *The pre-2023 cut* in `HISTORY.md`. The 422-talk backlog the seven new conferences created was fetched the same day — see *Closing the 422* in `HISTORY.md`. Eight are `hi` and cannot be improved — see *Bug 7 cannot be fixed by refetching* in `HISTORY.md`. What is left is pre-2026 and deliberately unfetched — see *Collection is scoped to 2026* in `ARCHITECTURE.md`. |
| Imports (`import_kb.py`) | Done for WeAreDevelopers World Congress 2026: 358 talks and their transcripts, from the private agenda-API corpus (`--kb`). Offline and rerunnable. |
| Workflows | `tests.yml` (lint + offline suites, 2026-09-11) and `pages.yml`. `kb-refresh.yml` was removed 2026-09-11 in favour of `tools/refresh_local.sh`, daily on this machine via systemd, because CI had no keys. `pages.yml` was rewritten on 2026-09-02: it now runs `tools/assemble_site.sh` and pushes only what the browser fetches to `gh-pages` as one orphan commit — 249 MB instead of the whole 411 MB repo. **Verified live 2026-09-02**: the merge of `infoq-presentations` was its first deploy — the assemble step reported 250 MB (transcripts 199 MB, tindex 46 MB, search-meta.json 5.7 MB), `gh-pages` is one orphan commit, and the `load` and `moments` suites pass 34 of 34 against the published site. `kb-refresh.yml` pushes a review branch instead of committing to `main`, after the run that did — see *The CI refresh regression* in `HISTORY.md`. |
...
fetched on 2026-09-01 (*Closing the 422* in `HISTORY.md`), the one the year-fallback fix
later moved into the scope went the same way (*The pre-2023 cut* in `HISTORY.md`), and
the 241 the 2026-09-06 refresh selected were fetched that day — 232 arrived, 9 are
facts about the video (*The refresh of 2026-09-06* in `HISTORY.md`). The
commands below are exactly what that run used, and the same shape as any future selection:
...
- **Check `_misses.json` after a big run**, and read the details rather than
  ...
  only way back.
- **34 of September's 3,000 credits are spent**, all of them on 2026-09-14 —
  `~/.config/ai-talks-universe/credits-2026-09.log`, which `refresh_local.sh`
  writes and reads. The billing month had reset before that run: it reported
  0 of 3,000 spent. Earlier spends are on the previous month's ledger (~2,370
  for the 422-talk run, ~242 on 2026-09-06). The dashboard is the authority;
  this file only records what was spent.
