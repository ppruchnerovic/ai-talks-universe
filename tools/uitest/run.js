...
const SITE = path.join(HERE, '..', '..');   // repo root — the site is served from there

const SUITES = fs.readdirSync(HERE).filter(f => /^suite-.*\.js$/.test(f)).sort();
...
const runSuite = (file, env) => new Promise(resolve => {
  ...
  child.on('close', code => {
    ...
    resolve(m
      ? { pass: +m[1], fail: +m[2], skip: +m[3] }
      // No tally line means the suite died before it could report one.
      : { pass: 0, fail: 1, skip: 0, crashed: true, code });
  });
});
...
(async () => {
  ...
  const [pass, fail, skip] = [sum('pass'), sum('fail'), sum('skip')];
  console.log('═'.repeat(70));
  for (const r of results) {
    console.log(`  ${r.name.padEnd(12)} ${String(r.pass).padStart(3)} pass  ` +
      `${String(r.fail).padStart(2)} fail  ${String(r.skip).padStart(2)} skip` +
      (r.crashed ? `   (suite exited ${r.code} without reporting)` : ''));
  }
  console.log('═'.repeat(70));
  console.log(`  ${pass + fail} checks, ${fail} failing` + (skip ? `, ${skip} skipped` : ''));
