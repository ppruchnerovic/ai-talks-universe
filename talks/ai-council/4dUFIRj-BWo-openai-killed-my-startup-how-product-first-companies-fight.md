---
id: 4dUFIRj-BWo
title: "OpenAI Killed My Startup - How Product-First Companies Fight Back"
slug: openai-killed-my-startup-how-product-first-companies-fight
conference: ai-council
conference_name: "AI Council (formerly Data Council)"
category: "Practitioner AI conferences"
edition: "Data Council / AI Council"
year: 2025
speakers: ["Han-chung Lee"]
channel: null
duration_min: 39
published_at: 2025-05-29T14:35:43Z
video_id: 4dUFIRj-BWo
url: https://www.youtube.com/watch?v=4dUFIRj-BWo
youtube_url: https://www.youtube.com/watch?v=4dUFIRj-BWo
tags: ["AI business", "product strategy", "model centric approach", "ChatGPT", "artificial intelligence", "machine learning", "competitive advantage", "tech strategy", "business model", "AI market", "innovation", "distribution moat", "AI economics", "model differentiation", "startup strategy", "Python", "AI product management", "go to market strategy", "AI adoption", "digital transformation", "business intelligence", "market analysis", "competitive intelligence", "strategic planning", "technology roadmap"]
topics: ["Classic ML & data science", "Enterprise adoption & strategy"]
transcript: false
---

# OpenAI Killed My Startup - How Product-First Companies Fight Back

**Han-chung Lee**

`AI Council (formerly Data Council)` · `Data Council / AI Council` · `2025` · `39 min`

`#AI business` `#product strategy` `#model centric approach` `#ChatGPT` `#artificial intelligence` `#machine learning` `#competitive advantage` `#tech strategy` `#business model` `#AI market` `#innovation` `#distribution moat` `#AI economics` `#model differentiation` `#startup strategy` `#Python` `#AI product management` `#go to market strategy` `#AI adoption` `#digital transformation` `#business intelligence` `#market analysis` `#competitive intelligence` `#strategic planning` `#technology roadmap`

[Watch the recording](https://www.youtube.com/watch?v=4dUFIRj-BWo) · [Conference site](https://www.aicouncil.com/)

## Description

[2025 - Day 2 - Foundation Models] Han-chung Lee shares insights from the evolution of intelligence as the primary product differentiator, exploring how the model has become the central selling point across technological eras. This talk offers valuable perspectives on the strategic bifurcation companies face and how "the model is the product, the distribution is the moat" is reshaping competitive dynamics across sectors, crucial for leaders building model-centric products or developing distribution strategies.

ABOUT THE SPEAKER:
Han-chung Lee, Machine Learning Director, Moody's Analytics

🎟️ GET YOUR TICKET TO AI COUNCIL 2026 🎟️
Meet the world's top AI infrastructure minds where architects of AI share what works. Three days of high-quality technical talks and meaningful interactions.

→ https://aicouncil.com/sf-2026

⚡ FIND US:
X: https://x.com/AICouncilConf
