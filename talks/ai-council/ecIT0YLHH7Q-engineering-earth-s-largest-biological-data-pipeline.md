---
id: ecIT0YLHH7Q
title: "Engineering Earth's Largest Biological Data Pipeline"
slug: engineering-earth-s-largest-biological-data-pipeline
conference: ai-council
conference_name: "AI Council (formerly Data Council)"
category: "AI engineering & agents"
edition: "Data Council / AI Council"
year: 2025
speakers: []
channel: "AI Council"
duration_min: 18
published_at: 2025-05-29T00:44:24Z
video_id: ecIT0YLHH7Q
url: https://www.youtube.com/watch?v=ecIT0YLHH7Q
youtube_url: https://www.youtube.com/watch?v=ecIT0YLHH7Q
tags: ["bioinformatics", "life sciences", "ChatGPT", "artificial intelligence", "machine learning", "data engineering", "big data", "biological data", "genomics", "computational biology", "digital twin", "foundation models", "Python tutorial", "sequence analysis", "structural biology", "systems biology", "proteomics", "metabolomics", "transcriptomics", "phylogenetics", "evolution", "drug discovery", "personalized medicine", "precision medicine", "clinical trials"]
topics: ["Data engineering & MLOps", "Science, healthcare & applied ML"]
transcript: false
---

# Engineering Earth's Largest Biological Data Pipeline

**Speaker not identified**

`AI Council (formerly Data Council)` · `Data Council / AI Council` · `2025` · `18 min`

`#bioinformatics` `#life sciences` `#ChatGPT` `#artificial intelligence` `#machine learning` `#data engineering` `#big data` `#biological data` `#genomics` `#computational biology` `#digital twin` `#foundation models` `#Python tutorial` `#sequence analysis` `#structural biology` `#systems biology` `#proteomics` `#metabolomics` `#transcriptomics` `#phylogenetics` `#evolution` `#drug discovery` `#personalized medicine` `#precision medicine` `#clinical trials`

[Watch the recording](https://www.youtube.com/watch?v=ecIT0YLHH7Q) · [Conference site](https://www.aicouncil.com/)

## Description

[2025 - Day 3 - Lightning Talks] Saif Ur-Rehman shares insights from building Basecamp Research's proprietary digital twin of life on Earth, exploring how their global biological data supply chain enables next-generation foundation models. This talk offers valuable perspectives on tackling the 99.9% knowledge gap in life sciences through heterogeneous multilayer data representations, crucial for teams working with complex experimental workflows and state-of-the-art AI models in biological research.

ABOUT THE SPEAKER:
Saif Ur-Rehman, Data Engineering Lead, Basecamp Research -

🎟️ GET YOUR TICKET TO AI COUNCIL 2026 🎟️
Meet the world's top AI infrastructure minds where architects of AI share what works. Three days of high-quality technical talks and meaningful interactions.

→ https://aicouncil.com/sf-2026

⚡ FIND US:
X: https://x.com/AICouncilConf
