---
id: Ig8nsmPlgb8
title: "Driving Revenue By Getting Your Data Outside Your Company"
slug: driving-revenue-by-getting-your-data-outside-your-company
conference: ai-council
conference_name: "AI Council (formerly Data Council)"
category: "AI engineering & agents"
edition: "Data Council / AI Council"
year: 2024
speakers: ["Solomon Kahn"]
channel: "AI Council"
duration_min: 29
published_at: 2024-04-19T05:00:08Z
video_id: Ig8nsmPlgb8
url: https://www.youtube.com/watch?v=Ig8nsmPlgb8
youtube_url: https://www.youtube.com/watch?v=Ig8nsmPlgb8
tags: ["machine learning", "computer vision", "AI"]
transcript: false
---

# Driving Revenue By Getting Your Data Outside Your Company

**Solomon Kahn**

`AI Council (formerly Data Council)` · `Data Council / AI Council` · `2024` · `29 min`

`#machine learning` `#computer vision` `#AI`

[Watch the recording](https://www.youtube.com/watch?v=Ig8nsmPlgb8) · [Conference site](https://www.aicouncil.com/)

## Description

Learn how to transform data into a revenue-generating asset and explore how data can be more than just a tool for internal decisions; it can become a valuable revenue stream. Find out the step-by-step processes, tools and techniques to launch successful data products that generate revenue. With real-life examples and practical advice, this video will guide you through visualizing and executing a data monetization strategy.

👉 Sign up for our "No BS" Newsletter to get the latest technical data & AI content: https://hubs.li/Q02vz6xC0

ABOUT THE SPEAKER:
Solomon Kahn, Founder & CEO, Delivery Layer (ex- Helium 20, Nielsen)

ABOUT DATA COUNCIL:
Data Council brings together the brightest minds in data to share industry knowledge, technical architectures and best practices in building cutting edge data & AI systems and tools.

FIND US:

🎟️ GET YOUR TICKET TO AI COUNCIL 2026 🎟️
Meet the world's top AI infrastructure minds where architects of AI share what works. Three days of high-quality technical talks and meaningful interactions.

→ https://aicouncil.com/sf-2026

⚡ FIND US:
X: https://x.com/AICouncilConf
