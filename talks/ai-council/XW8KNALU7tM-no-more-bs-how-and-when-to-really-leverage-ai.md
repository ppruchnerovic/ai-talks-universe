---
id: XW8KNALU7tM
title: "No More BS: How and When to Really Leverage AI"
slug: no-more-bs-how-and-when-to-really-leverage-ai
conference: ai-council
conference_name: "AI Council (formerly Data Council)"
category: "Practitioner AI conferences"
edition: "Data Council / AI Council"
year: 2025
speakers: ["Colleen Tartow"]
channel: null
duration_min: 26
published_at: 2025-05-29T00:09:30Z
video_id: XW8KNALU7tM
url: https://www.youtube.com/watch?v=XW8KNALU7tM
youtube_url: https://www.youtube.com/watch?v=XW8KNALU7tM
tags: ["critical thinking", "AI strategy", "practical AI", "ChatGPT", "artificial intelligence", "machine learning", "AI implementation", "business AI", "AI adoption", "data quality", "data governance", "AI framework", "Python tutorial", "strategic planning", "decision making", "problem solving", "analytical thinking", "business strategy", "technology strategy", "risk assessment", "change management", "stakeholder management", "project management"]
topics: ["Enterprise adoption & strategy"]
transcript: false
---

# No More BS: How and When to Really Leverage AI

**Colleen Tartow**

`AI Council (formerly Data Council)` · `Data Council / AI Council` · `2025` · `26 min`

`#critical thinking` `#AI strategy` `#practical AI` `#ChatGPT` `#artificial intelligence` `#machine learning` `#AI implementation` `#business AI` `#AI adoption` `#data quality` `#data governance` `#AI framework` `#Python tutorial` `#strategic planning` `#decision making` `#problem solving` `#analytical thinking` `#business strategy` `#technology strategy` `#risk assessment` `#change management` `#stakeholder management` `#project management`

[Watch the recording](https://www.youtube.com/watch?v=XW8KNALU7tM) · [Conference site](https://www.aicouncil.com/)

## Description

[2025 - Day 3 - AI & Data Culture] Colleen Tartow and Lindsay Murphy share insights from practical AI implementation frameworks, exploring how to assess whether AI is truly the optimal solution versus simpler, established methods. For practitioners navigating AI hype while maintaining critical thinking, this data-driven conversation offers valuable perspectives on building solid foundations of data quality and governance before pursuing AI solutions, emphasizing practical considerations over technological excitement.

ABOUT THE SPEAKERS:
Colleen Tartow, Senior Director Enterprise Data Engineering, Capital One
Lindsay Murphy, Director & Head of Data, Hiive -

🎟️ GET YOUR TICKET TO AI COUNCIL 2026 🎟️
Meet the world's top AI infrastructure minds where architects of AI share what works. Three days of high-quality technical talks and meaningful interactions.

→ https://aicouncil.com/sf-2026

⚡ FIND US:
X: https://x.com/AICouncilConf
