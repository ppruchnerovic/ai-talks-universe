---
id: WV7-VsgOIr4
title: "OttoBot: Productionizing LLM Models"
slug: ottobot-productionizing-llm-models
conference: ai-council
conference_name: "AI Council (formerly Data Council)"
category: "Practitioner AI conferences"
edition: "Data Council / AI Council"
year: 2024
speakers: []
channel: "AI Council"
duration_min: 36
published_at: 2024-04-19T05:00:28Z
video_id: WV7-VsgOIr4
url: https://www.youtube.com/watch?v=WV7-VsgOIr4
youtube_url: https://www.youtube.com/watch?v=WV7-VsgOIr4
tags: ["machine learning", "computer vision", "AI"]
topics: []
transcript: false
---

# OttoBot: Productionizing LLM Models

**Speaker not identified**

`AI Council (formerly Data Council)` · `Data Council / AI Council` · `2024` · `36 min`

`#machine learning` `#computer vision` `#AI`

[Watch the recording](https://www.youtube.com/watch?v=WV7-VsgOIr4) · [Conference site](https://www.aicouncil.com/)

## Description

Unravel the intricacies of deploying Large Language Models (LLMs) in production with this insightful talk by Weights & Biases CEO, Lukas Biewald. As a leader of an AI Platform trusted by tech giants like OpenAI, NVIDIA and Meta, Lukas is well-equipped to guide you through the process. This definitive guide includes case studies, industry best practices, key tools and troubleshooting techniques for common challenges. Witness industry-leading data scientists transform LLM models into powerful applications firsthand. Tune in for a compelling exploration of LLM production with diverse industry examples.

👉  Sign up for our "No BS" Newsletter to get the latest technical data & AI content: https://hubs.li/Q02vz6xC0

ABOUT DATA COUNCIL:
Data Council brings together the brightest minds in data to share industry knowledge, technical architectures and best practices in building cutting edge data & AI systems and tools.

FIND US:

#largelanguagemodels -

🎟️ GET YOUR TICKET TO AI COUNCIL 2026 🎟️
Meet the world's top AI infrastructure minds where architects of AI share what works. Three days of high-quality technical talks and meaningful interactions.

→ https://aicouncil.com/sf-2026

⚡ FIND US:
X: https://x.com/AICouncilConf
