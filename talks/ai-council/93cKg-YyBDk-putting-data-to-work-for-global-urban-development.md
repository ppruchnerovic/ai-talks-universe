---
id: 93cKg-YyBDk
title: "Putting Data To Work For Global Urban Development"
slug: putting-data-to-work-for-global-urban-development
conference: ai-council
conference_name: "AI Council (formerly Data Council)"
category: "AI engineering & agents"
edition: "Data Council / AI Council"
year: 2025
speakers: []
channel: "AI Council"
duration_min: 18
published_at: 2025-05-29T00:43:27Z
video_id: 93cKg-YyBDk
youtube_url: https://www.youtube.com/watch?v=93cKg-YyBDk
tags: ["smart cities", "data science", "ChatGPT", "artificial intelligence", "machine learning", "policy", "social impact", "urban data", "urban analytics", "satellite imagery", "LLM datasets", "Python tutorial", "urban planning", "transportation analytics", "traffic optimization", "energy management", "environmental monitoring", "air quality", "waste management", "water management", "public safety", "emergency response", "citizen services", "digital governance"]
transcript: false
---

# Putting Data To Work For Global Urban Development

**Speaker not identified**

`AI Council (formerly Data Council)` · `Data Council / AI Council` · `2025` · `18 min`

`#smart cities` `#data science` `#ChatGPT` `#artificial intelligence` `#machine learning` `#policy` `#social impact` `#urban data` `#urban analytics` `#satellite imagery` `#LLM datasets` `#Python tutorial` `#urban planning` `#transportation analytics` `#traffic optimization` `#energy management` `#environmental monitoring` `#air quality` `#waste management` `#water management` `#public safety` `#emergency response` `#citizen services` `#digital governance`

[Watch the recording](https://www.youtube.com/watch?v=93cKg-YyBDk) · [Conference site](https://www.aicouncil.com/)

## Description

[2025 - Day 3 - Lightning Talks] Tobias Lunt shares insights from harnessing modern data and AI tooling to improve lives in the developing world, exploring how real-time urban data can inform smart policy and planning decisions. For data practitioners interested in public good applications, this talk offers valuable perspectives on combining incremental improvements with tectonic shifts like AI-assisted development and LLM toolchains to deliver outsized impact for global urban challenges.

ABOUT THE SPEAKER:
Tobias Lunt, Co-Founder & Data Scientist, Development Data Lab -

🎟️ GET YOUR TICKET TO AI COUNCIL 2026 🎟️
Meet the world's top AI infrastructure minds where architects of AI share what works. Three days of high-quality technical talks and meaningful interactions.

→ https://aicouncil.com/sf-2026

⚡ FIND US:
X: https://x.com/AICouncilConf
