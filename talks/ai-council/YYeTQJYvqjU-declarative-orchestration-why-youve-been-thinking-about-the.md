---
id: YYeTQJYvqjU
title: "Declarative Orchestration: Why You’ve Been Thinking About the Wrong DAG the Entire Time"
slug: declarative-orchestration-why-youve-been-thinking-about-the
conference: ai-council
conference_name: "AI Council (formerly Data Council)"
category: "Practitioner AI conferences"
edition: "Data Council / AI Council"
year: 2024
speakers: ["Pete Hunt"]
channel: "AI Council"
duration_min: 32
published_at: 2024-04-17T05:00:13Z
video_id: YYeTQJYvqjU
url: https://www.youtube.com/watch?v=YYeTQJYvqjU
youtube_url: https://www.youtube.com/watch?v=YYeTQJYvqjU
tags: ["machine learning", "computer vision", "AI"]
topics: []
transcript: false
---

# Declarative Orchestration: Why You’ve Been Thinking About the Wrong DAG the Entire Time

**Pete Hunt**

`AI Council (formerly Data Council)` · `Data Council / AI Council` · `2024` · `32 min`

`#machine learning` `#computer vision` `#AI`

[Watch the recording](https://www.youtube.com/watch?v=YYeTQJYvqjU) · [Conference site](https://www.aicouncil.com/)

## Description

Join Pete Hunt, CEO of Dagster Labs, as he unveils the game-changing power of declarative, asset-oriented orchestration in data pipelines. In this video, Pete breaks down how this approach revolutionizes your data platform, simplifying tasks for developers while ensuring data integrity.

Discover the concept of a Data Mesh and its practical implementation as a living asset graph within the product, leveraging shared infrastructure.

👉 Sign up for our "No BS" Newsletter to get the latest technical data & AI content: https://hubs.li/Q02vz6xC0

ABOUT THE SPEAKER:
Pete Hunt, CEO, Dagster Labs (ex- Smyte; CEO/Co-Founder)

ABOUT DATA COUNCIL:
Data Council brings together the brightest minds in data to share industry knowledge, technical architectures and best practices in building cutting edge data & AI systems and tools.

FIND US:

🎟️ GET YOUR TICKET TO AI COUNCIL 2026 🎟️
Meet the world's top AI infrastructure minds where architects of AI share what works. Three days of high-quality technical talks and meaningful interactions.

→ https://aicouncil.com/sf-2026

⚡ FIND US:
X: https://x.com/AICouncilConf
