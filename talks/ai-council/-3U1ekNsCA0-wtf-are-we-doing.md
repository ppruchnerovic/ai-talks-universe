---
id: -3U1ekNsCA0
title: "WTF Are We Doing?"
slug: wtf-are-we-doing
conference: ai-council
conference_name: "AI Council (formerly Data Council)"
category: "Practitioner AI conferences"
edition: "Data Council / AI Council"
year: 2024
speakers: ["Benn Stancil"]
channel: null
duration_min: 32
published_at: 2024-04-19T05:00:00Z
video_id: -3U1ekNsCA0
url: https://www.youtube.com/watch?v=-3U1ekNsCA0
youtube_url: https://www.youtube.com/watch?v=-3U1ekNsCA0
tags: ["machine learning", "computer vision", "AI"]
topics: ["Enterprise adoption & strategy"]
transcript: false
---

# WTF Are We Doing?

**Benn Stancil**

`AI Council (formerly Data Council)` · `Data Council / AI Council` · `2024` · `32 min`

`#machine learning` `#computer vision` `#AI`

[Watch the recording](https://www.youtube.com/watch?v=-3U1ekNsCA0) · [Conference site](https://www.aicouncil.com/)

## Description

Explore the effectiveness and impact of data-focused communities and the journey towards fostering a robust data culture. Are we truly progressing, or simply caught in the hype? Can advancements in AI and data tooling empower us to build high-impact data teams? Discover insights from our distinguished panel as they dig into these questions and more.

👉 Sign up for our "No BS" Newsletter to get the latest technical data & AI content: https://hubs.li/Q02vz6xC0

ABOUT THE SPEAKERS:
Benn Stancil, Field CTO, ThoughtSpot (ex- Microsoft, Yammer)
Chad Sanderson, CEO, Gable.ai (ex- Head of Data @ Convoy)
Caitlin Hudson, Data Scientist, Figma (with 10+ years of learning from data mistakes)
Moderator: Maggie Hays, Founding Community Product Manager, Datahub

ABOUT DATA COUNCIL:
Data Council brings together the brightest minds in data to share industry knowledge, technical architectures and best practices in building cutting edge data & AI systems and tools.

FIND US:

🎟️ GET YOUR TICKET TO AI COUNCIL 2026 🎟️
Meet the world's top AI infrastructure minds where architects of AI share what works. Three days of high-quality technical talks and meaningful interactions.

→ https://aicouncil.com/sf-2026

⚡ FIND US:
X: https://x.com/AICouncilConf
