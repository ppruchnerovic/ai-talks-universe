---
id: hYpl0mVPkbc
title: "The Art of Data: Reimagining Creative Processes with Data Culture"
slug: the-art-of-data-reimagining-creative-processes-with-data
conference: ai-council
conference_name: "AI Council (formerly Data Council)"
category: "AI engineering & agents"
edition: "Data Council / AI Council"
year: 2025
speakers: []
channel: "AI Council"
duration_min: 16
published_at: 2025-05-29T00:39:22Z
video_id: hYpl0mVPkbc
youtube_url: https://www.youtube.com/watch?v=hYpl0mVPkbc
tags: ["data visualization", "creative coding", "design", "ChatGPT", "artificial intelligence", "machine learning", "data storytelling", "data art", "data culture", "creative processes", "artistic creativity", "data literacy", "collaboration", "Python tutorial", "information design", "visual analytics", "interactive visualization", "dashboard design", "chart design", "color theory", "typography", "user experience design", "accessibility", "responsive design"]
transcript: false
---

# The Art of Data: Reimagining Creative Processes with Data Culture

**Speaker not identified**

`AI Council (formerly Data Council)` · `Data Council / AI Council` · `2025` · `16 min`

`#data visualization` `#creative coding` `#design` `#ChatGPT` `#artificial intelligence` `#machine learning` `#data storytelling` `#data art` `#data culture` `#creative processes` `#artistic creativity` `#data literacy` `#collaboration` `#Python tutorial` `#information design` `#visual analytics` `#interactive visualization` `#dashboard design` `#chart design` `#color theory` `#typography` `#user experience design` `#accessibility` `#responsive design`

[Watch the recording](https://www.youtube.com/watch?v=hYpl0mVPkbc) · [Conference site](https://www.aicouncil.com/)

## Description

[2025 - Day 3 - Lightning Talks] Michael Cohen shares insights from bridging the gap between artistic creativity and data intelligence, exploring strategies to foster a culture where data fuels rather than stifles creative expression. For leaders working across creative and analytical teams, this talk offers valuable perspectives on overcoming perception barriers, fostering data literacy among creatives, and implementing data storytelling approaches that enhance impactful creativity and collaboration.

ABOUT THE SPEAKER:
Michael Cohen, Global Chief Data & Analytics Officer, Plus Company -

🎟️ GET YOUR TICKET TO AI COUNCIL 2026 🎟️
Meet the world's top AI infrastructure minds where architects of AI share what works. Three days of high-quality technical talks and meaningful interactions.

→ https://aicouncil.com/sf-2026

⚡ FIND US:
X: https://x.com/AICouncilConf
