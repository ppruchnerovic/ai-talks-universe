---
id: Bugs0dVcNI8
title: "How Uber Built AI Agents That Save 21,000 Developer Hours with LangGraph | LangChain Interrupt"
slug: how-uber-built-ai-agents-that-save-21-000-developer-hours
conference: langchain-interrupt
conference_name: "LangChain Interrupt"
category: "AI engineering & agents"
edition: "Interrupt 2025"
year: 2025
speakers: []
channel: "LangChain"
duration_min: 17
published_at: null
video_id: Bugs0dVcNI8
youtube_url: https://www.youtube.com/watch?v=Bugs0dVcNI8
tags: []
transcript: false
---

# How Uber Built AI Agents That Save 21,000 Developer Hours with LangGraph | LangChain Interrupt

**Speaker not identified**

`LangChain Interrupt` · `Interrupt 2025` · `2025` · `17 min`

[Watch the recording](https://www.youtube.com/watch?v=Bugs0dVcNI8) · [Conference site](https://interrupt.langchain.com/)

## Description

*No description published on YouTube.*
