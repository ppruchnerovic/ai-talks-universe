---
id: TPpxkVCbL4c
title: "PG - The Current State of Adversarial Machine Learning - Heather Lawrence"
slug: pg-the-current-state-of-adversarial-machine-learning
conference: bsides-lv
conference_name: "BSides Las Vegas"
category: "Security conferences"
edition: "BSides Las Vegas"
year: 2018
speakers: ["Heather Lawrence"]
channel: "BSidesLV"
duration_min: 22
published_at: 2018-09-20T18:27:38Z
video_id: TPpxkVCbL4c
url: https://www.youtube.com/watch?v=TPpxkVCbL4c
youtube_url: https://www.youtube.com/watch?v=TPpxkVCbL4c
tags: ["bslv", "bslv2018", "bsideslv", "bsideslv2018", "pg", "proving ground"]
topics: ["Classic ML & data science", "Security, safety & red teaming"]
transcript: false
---

# PG - The Current State of Adversarial Machine Learning - Heather Lawrence

**Heather Lawrence**

`BSides Las Vegas` · `BSides Las Vegas` · `2018` · `22 min`

`#bslv` `#bslv2018` `#bsideslv` `#bsideslv2018` `#pg` `#proving ground`

[Watch the recording](https://www.youtube.com/watch?v=TPpxkVCbL4c) · [Conference site](https://bsideslv.org/)

## Description

The Current State of Adversarial Machine Learning - Heather Lawrence
Proving Ground
BSidesLV 2018 - Tuscany Hotel - Aug 08, 2018
