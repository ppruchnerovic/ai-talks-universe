---
id: ReKVGEjWDM4
title: "GT - Security data science -- Getting the Fundamentals Right - Richard Harang"
slug: gt-security-data-science-getting-the-fundamentals-right
conference: bsides-lv
conference_name: "BSides Las Vegas"
category: "Security conferences"
edition: "BSides Las Vegas"
year: 2019
speakers: ["Richard Harang"]
channel: null
duration_min: 51
published_at: 2019-10-19T17:54:01Z
video_id: ReKVGEjWDM4
url: https://www.youtube.com/watch?v=ReKVGEjWDM4
youtube_url: https://www.youtube.com/watch?v=ReKVGEjWDM4
tags: ["bslv", "BSidesLV", "bslv2019", "2019-bsideslv-049", "Ground Truth", "GT", "gt", "BSidesLV2019"]
topics: ["Classic ML & data science", "Security, safety & red teaming"]
transcript: false
---

# GT - Security data science -- Getting the Fundamentals Right - Richard Harang

**Richard Harang**

`BSides Las Vegas` · `BSides Las Vegas` · `2019` · `51 min`

`#bslv` `#BSidesLV` `#bslv2019` `#2019-bsideslv-049` `#Ground Truth` `#GT` `#gt` `#BSidesLV2019`

[Watch the recording](https://www.youtube.com/watch?v=ReKVGEjWDM4) · [Conference site](https://bsideslv.org/)

## Description

GT - Security data science -- Getting the Fundamentals Right - Richard Harang
Ground Truth
BSidesLV 2019 - Tuscany Hotel - Aug 07, 2019
