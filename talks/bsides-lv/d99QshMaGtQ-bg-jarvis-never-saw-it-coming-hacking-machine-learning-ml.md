---
id: d99QshMaGtQ
title: "BG - JARVIS Never Saw It Coming: Hacking machine learning (ML) in Speech, Text and Face Recognition"
slug: bg-jarvis-never-saw-it-coming-hacking-machine-learning-ml
conference: bsides-lv
conference_name: "BSides Las Vegas"
category: "AI security"
edition: "BSides Las Vegas"
year: 2018
speakers: []
channel: "BSidesLV"
duration_min: 50
published_at: 2018-09-26T22:36:28Z
video_id: d99QshMaGtQ
youtube_url: https://www.youtube.com/watch?v=d99QshMaGtQ
tags: ["bslv", "bslv2018", "bsideslv", "bsideslv2018", "bg", "breaking ground"]
transcript: false
---

# BG - JARVIS Never Saw It Coming: Hacking machine learning (ML) in Speech, Text and Face Recognition

**Speaker not identified**

`BSides Las Vegas` · `BSides Las Vegas` · `2018` · `50 min`

`#bslv` `#bslv2018` `#bsideslv` `#bsideslv2018` `#bg` `#breaking ground`

[Watch the recording](https://www.youtube.com/watch?v=d99QshMaGtQ) · [Conference site](https://bsideslv.org/)

## Description

JARVIS Never Saw It Coming: Hacking machine learning (ML) in Speech, Text and Face Recognition - and frankly, everywhere else - Guy Barnhart-Magen & Ezra Caltum
Breaking Ground
BSidesLV 2018 - Tuscany Hotel - Aug 07, 2018
