---
id: FXbdANtUE_k
title: "ProblemChild: Discovering Anomalous Patterns based on Parent-Child Process Relationships"
slug: problemchild-discovering-anomalous-patterns-based-on-parent
conference: camlis
conference_name: "CAMLIS"
category: "Security conferences"
edition: "CAMLIS"
year: 2019
speakers: []
channel: null
duration_min: 30
published_at: 2019-11-12T22:47:35Z
video_id: FXbdANtUE_k
url: https://www.youtube.com/watch?v=FXbdANtUE_k
youtube_url: https://www.youtube.com/watch?v=FXbdANtUE_k
tags: ["camlis", "camlis2019"]
topics: []
transcript: false
---

# ProblemChild: Discovering Anomalous Patterns based on Parent-Child Process Relationships

**Speaker not identified**

`CAMLIS` · `CAMLIS` · `2019` · `30 min`

`#camlis` `#camlis2019`

[Watch the recording](https://www.youtube.com/watch?v=FXbdANtUE_k) · [Conference site](https://www.camlis.org/)

## Description

CAMLIS 2019, Bobby Filar
ProblemChild: Discovering Anomalous Patterns based on Parent-Child Process Relationships (abstract: https://www.camlis.org/2019/talks/filar)
