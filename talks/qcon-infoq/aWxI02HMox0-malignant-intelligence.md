---
id: aWxI02HMox0
title: "Malignant Intelligence?"
slug: malignant-intelligence
conference: qcon-infoq
conference_name: "QCon / InfoQ Dev Summit"
category: "Software dev with AI tracks"
edition: "InfoQ"
year: 2023
speakers: []
channel: null
duration_min: 54
published_at: 2023-10-23T08:50:29Z
video_id: aWxI02HMox0
youtube_url: https://www.youtube.com/watch?v=aWxI02HMox0
tags: ["Software Development", "Software Architecture", "Artificial Intelligence", "Machine Learning", "Data Engineering", "Java", "Ethics", "InfoQ", "QCon London", "Transcript"]
transcript: false
---

# Malignant Intelligence?

**Speaker not identified**

`QCon / InfoQ Dev Summit` · `InfoQ` · `2023` · `54 min`

`#Software Development` `#Software Architecture` `#Artificial Intelligence` `#Machine Learning` `#Data Engineering` `#Java` `#Ethics` `#InfoQ` `#QCon London` `#Transcript`

[Watch the recording](https://www.youtube.com/watch?v=aWxI02HMox0) · [Conference site](https://qconferences.com/)

## Description

Video with transcript included on InfoQ: https://bit.ly/4985K2n

Alasdair Allen discusses the potentially ethical dilemmas, new security concerns, and open questions about the future of software development in the era of machine learning.

-----------------------------------------------------------------------------------------------------
Follow InfoQ:
- Mastodon: https://techhub.social/@infoq
- Twitter: https://www.twitter.com/infoq
- LinkedIn: https://www.linkedin.com/company/infoq/
- Facebook: https://www.facebook.com/InfoQdotcom/
- Instagram: @infoqdotcom
