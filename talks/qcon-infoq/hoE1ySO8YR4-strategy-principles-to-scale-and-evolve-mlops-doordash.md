---
id: hoE1ySO8YR4
title: "Strategy & Principles to Scale and Evolve MLOps @DoorDash"
slug: strategy-principles-to-scale-and-evolve-mlops-doordash
conference: qcon-infoq
conference_name: "QCon / InfoQ Dev Summit"
category: "Software dev with AI tracks"
edition: "InfoQ"
year: 2023
speakers: []
channel: "InfoQ"
duration_min: 42
published_at: 2023-12-07T06:36:16Z
video_id: hoE1ySO8YR4
url: https://www.youtube.com/watch?v=hoE1ySO8YR4
youtube_url: https://www.youtube.com/watch?v=hoE1ySO8YR4
tags: ["Artificial Intelligence", "Machine Learning", "Data Engineering", "MLOps", "Case Study", "DoorDash", "Project Management", "Operations Management", "InfoQ", "QCon London", "Transcript"]
topics: ["Data engineering & MLOps"]
transcript: false
---

# Strategy & Principles to Scale and Evolve MLOps @DoorDash

**Speaker not identified**

`QCon / InfoQ Dev Summit` · `InfoQ` · `2023` · `42 min`

`#Artificial Intelligence` `#Machine Learning` `#Data Engineering` `#MLOps` `#Case Study` `#DoorDash` `#Project Management` `#Operations Management` `#InfoQ` `#QCon London` `#Transcript`

[Watch the recording](https://www.youtube.com/watch?v=hoE1ySO8YR4) · [Conference site](https://qconferences.com/)

## Description

Video with transcript included on InfoQ: https://bit.ly/3GDeu3r

Hien Luu shares their approach to MLOps, and the strategy and principles that have helped them to scale and evolve their platform to support hundreds of models and billions of predictions per day.

----------------------------------------------------------------------------------------------------
Follow InfoQ:
- Mastodon: https://techhub.social/@infoq
- Twitter: https://www.twitter.com/infoq
- LinkedIn: https://www.linkedin.com/company/infoq/
- Facebook: https://www.facebook.com/InfoQdotcom/
- Instagram: @infoqdotcom
