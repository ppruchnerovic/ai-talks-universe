---
id: 67uxiGfCWzY
title: "Applying AI to the SDLC: New Ideas and Gotchas! - Leveraging AI to Improve Software Engineering"
slug: applying-ai-to-the-sdlc-new-ideas-and-gotchas-leveraging-ai
conference: qcon-infoq
conference_name: "QCon / InfoQ Dev Summit"
category: "Software dev with AI tracks"
edition: "InfoQ"
year: 2024
speakers: []
channel: null
duration_min: 50
published_at: 2024-09-19T12:56:35Z
video_id: 67uxiGfCWzY
youtube_url: https://www.youtube.com/watch?v=67uxiGfCWzY
tags: ["Artificial Intelligence", "Machine Learning", "Data Engineering", "Generative AI", "Software Engineering", "DevSecOps", "Software Development Lifecycle", "Software Development", "InfoQ", "QCon London", "Transcript"]
transcript: false
---

# Applying AI to the SDLC: New Ideas and Gotchas! - Leveraging AI to Improve Software Engineering

**Speaker not identified**

`QCon / InfoQ Dev Summit` · `InfoQ` · `2024` · `50 min`

`#Artificial Intelligence` `#Machine Learning` `#Data Engineering` `#Generative AI` `#Software Engineering` `#DevSecOps` `#Software Development Lifecycle` `#Software Development` `#InfoQ` `#QCon London` `#Transcript`

[Watch the recording](https://www.youtube.com/watch?v=67uxiGfCWzY) · [Conference site](https://qconferences.com/)

## Description

Video with transcript included on InfoQ: http://bit.ly/3Zs2JYj

Tracy Bannon discusses using Generative AI in software engineering with AI-assistance to meet the speed and quality of end-users demand.

-----------------------------------------------------------------------------------
Follow InfoQ:
- Mastodon: https://techhub.social/@infoq
- Twitter: https://www.twitter.com/infoq
- LinkedIn: https://www.linkedin.com/company/infoq/
- Facebook: https://www.facebook.com/InfoQdotcom/
- Instagram: @infoqdotcom
