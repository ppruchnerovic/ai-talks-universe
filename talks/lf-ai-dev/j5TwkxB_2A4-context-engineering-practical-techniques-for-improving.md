---
id: j5TwkxB_2A4
title: "Context Engineering: Practical Techniques for Improving Agent Quality Today - Vaibhav Gupta"
slug: context-engineering-practical-techniques-for-improving
conference: lf-ai-dev
conference_name: "AI_dev / Open Source Summit (Linux Foundation)"
category: "General software conferences"
edition: "AI_dev Europe 2025"
year: 2025
speakers: ["Vaibhav Gupta"]
channel: "The Linux Foundation"
duration_min: 26
published_at: 2025-09-09T18:27:22Z
video_id: j5TwkxB_2A4
url: https://www.youtube.com/watch?v=j5TwkxB_2A4
youtube_url: https://www.youtube.com/watch?v=j5TwkxB_2A4
tags: []
topics: ["Agents & orchestration", "Prompting & context engineering"]
transcript: false
---

# Context Engineering: Practical Techniques for Improving Agent Quality Today - Vaibhav Gupta

**Vaibhav Gupta**

`AI_dev / Open Source Summit (Linux Foundation)` · `AI_dev Europe 2025` · `2025` · `26 min`

[Watch the recording](https://www.youtube.com/watch?v=j5TwkxB_2A4) · [Conference site](https://events.linuxfoundation.org/ai-dev-europe/)

## Description

Don't miss out! Join us at the next Open Source Summit in Seoul, South Korea (November 4-5). Join us at the premier vendor-neutral open source conference, where developers and technologists come together to collaborate, share knowledge, and explore the latest innovations and advancements in open source technology. Learn more at https://events.linuxfoundation.org/

Context Engineering: Practical Techniques for Improving Agent Quality Today - Vaibhav Gupta, Boundary
