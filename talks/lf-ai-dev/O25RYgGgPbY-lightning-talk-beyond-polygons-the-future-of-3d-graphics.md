---
id: O25RYgGgPbY
title: "Lightning Talk: Beyond Polygons: The Future of 3D Graphics with Implicit Models - Pierre Wilmot"
slug: lightning-talk-beyond-polygons-the-future-of-3d-graphics
conference: lf-ai-dev
conference_name: "AI_dev / Open Source Summit (Linux Foundation)"
category: "Software dev with AI tracks"
edition: "AI_dev Europe 2024"
year: 2024
speakers: ["Pierre Wilmot"]
channel: "The Linux Foundation"
duration_min: 20
published_at: 2024-06-27T14:39:45Z
video_id: O25RYgGgPbY
youtube_url: https://www.youtube.com/watch?v=O25RYgGgPbY
tags: []
transcript: false
---

# Lightning Talk: Beyond Polygons: The Future of 3D Graphics with Implicit Models - Pierre Wilmot

**Pierre Wilmot**

`AI_dev / Open Source Summit (Linux Foundation)` · `AI_dev Europe 2024` · `2024` · `20 min`

[Watch the recording](https://www.youtube.com/watch?v=O25RYgGgPbY) · [Conference site](https://events.linuxfoundation.org/ai-dev-europe/)

## Description

Lightning Talk: Beyond Polygons: The Future of 3D Graphics with Implicit Models - Pierre Wilmot, Dimension Studio

Exploring the landscape of implicit representation for 3D or 4D (3D over time) scenes representation. Neural radiance field, neural signed distance fonctions, adding motion over time, relighting, and so on.
