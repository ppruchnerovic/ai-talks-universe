---
id: y5g7u3sWyrk
title: "Keynote: The Post-Modern Data Stack for AI: Open Source Foundations for a Multimodal La... Chang She"
slug: keynote-the-post-modern-data-stack-for-ai-open-source
conference: lf-ai-dev
conference_name: "AI_dev / Open Source Summit (Linux Foundation)"
category: "Software dev with AI tracks"
edition: "AI_dev Europe 2025"
year: 2025
speakers: []
channel: "The Linux Foundation"
duration_min: 16
published_at: 2025-09-09T18:29:25Z
video_id: y5g7u3sWyrk
url: https://www.youtube.com/watch?v=y5g7u3sWyrk
youtube_url: https://www.youtube.com/watch?v=y5g7u3sWyrk
tags: []
transcript: false
---

# Keynote: The Post-Modern Data Stack for AI: Open Source Foundations for a Multimodal La... Chang She

**Speaker not identified**

`AI_dev / Open Source Summit (Linux Foundation)` · `AI_dev Europe 2025` · `2025` · `16 min`

[Watch the recording](https://www.youtube.com/watch?v=y5g7u3sWyrk) · [Conference site](https://events.linuxfoundation.org/ai-dev-europe/)

## Description

Don't miss out! Join us at the next Open Source Summit in Seoul, South Korea (November 4-5). Join us at the premier vendor-neutral open source conference, where developers and technologists come together to collaborate, share knowledge, and explore the latest innovations and advancements in open source technology. Learn more at https://events.linuxfoundation.org/

Keynote: The Post-Modern Data Stack for AI: Open Source Foundations for a Multimodal Lakehouse - Chang She, CEO & Co-founder, LanceDB
