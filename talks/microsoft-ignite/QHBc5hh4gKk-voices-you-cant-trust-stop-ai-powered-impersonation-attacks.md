---
id: QHBc5hh4gKk
title: "Voices you can’t trust: Stop AI-powered impersonation attacks | BRKSP487"
slug: voices-you-cant-trust-stop-ai-powered-impersonation-attacks
conference: microsoft-ignite
conference_name: "Microsoft Ignite"
category: "Vendor events"
edition: "Ignite 2025"
year: 2025
speakers: ["Jeremy Nelson"]
channel: "Microsoft Events"
duration_min: 45
published_at: 2025-11-21T10:53:42Z
video_id: QHBc5hh4gKk
url: https://www.youtube.com/watch?v=QHBc5hh4gKk
youtube_url: https://www.youtube.com/watch?v=QHBc5hh4gKk
tags: ["BRKSP487", "English (US)", "Financial Services", "Jeremy Nelson", "Protect cloud AI platform and apps", "Technical", "Voices you can’t trust: Stop AI-powered impersonation attacks | BRKSP487", "c3367214-ae5b-410e-bf9b-fd61c2a494f6_Segment", "f5n1", "ignite", "ignite 2025", "microsoft", "microsoft ignite", "microsoft ignite 2025", "ms ignite", "ms ignite 2025", "msft ignite", "msft ignite 2025"]
topics: ["Enterprise adoption & strategy", "Security, safety & red teaming"]
transcript: false
---

# Voices you can’t trust: Stop AI-powered impersonation attacks | BRKSP487

**Jeremy Nelson**

`Microsoft Ignite` · `Ignite 2025` · `2025` · `45 min`

`#BRKSP487` `#English (US)` `#Financial Services` `#Jeremy Nelson` `#Protect cloud AI platform and apps` `#Technical` `#Voices you can’t trust: Stop AI-powered impersonation attacks | BRKSP487` `#c3367214-ae5b-410e-bf9b-fd61c2a494f6_Segment` `#f5n1` `#ignite` `#ignite 2025` `#microsoft` `#microsoft ignite` `#microsoft ignite 2025` `#ms ignite` `#ms ignite 2025` `#msft ignite` `#msft ignite 2025`

[Watch the recording](https://www.youtube.com/watch?v=QHBc5hh4gKk) · [Conference site](https://ignite.microsoft.com/)

## Description

AI deepfakes are enabling fraud now, not in the future. Jeremy Nelson shares real cases, including a $25M scam from a fake video call, showing how threat actors bypass identity checks. Learn how Verified ID secures support desk interactions and get practical steps to protect your organization from AI-driven impersonation attacks.

𝗦𝗽𝗲𝗮𝗸𝗲𝗿𝘀:
* Jeremy Nelson

𝗦𝗲𝘀𝘀𝗶𝗼𝗻 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻:
This is one of many sessions from the Microsoft Ignite 2025 event. View even more sessions on-demand and learn about Microsoft Ignite at https://ignite.microsoft.com

BRKSP487 | English (US)

Breakout

#MSIgnite, #ProtectcloudAIplatformandapps

Chapters:
0:00 - Energetic Session Introduction at Microsoft Ignite
00:05:49 - Real-World Breach Case Studies: Clorox, Hong Kong Deepfake Scam, and KnowBe4
00:16:09 - Call Ends with Successful Social Engineering Attempt
00:21:27 - AI’s Role in Evolving Social Engineering and Attack Predictions
00:23:19 - High-risk targets: executives and public figures vulnerable to AI impersonation
00:24:14 - Deepfake social engineering and the asymmetry of attack success
00:25:47 - Introduction to Verified ID and decentralized identity concepts
00:33:38 - Introduction to verified ID process at Woodgrove support desk
00:42:38 - Importance of organizational change management, training, and communication
