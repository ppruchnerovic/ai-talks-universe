---
id: Zzk5LEwy7w4
title: "The Future of Power Platform: Intelligent Apps | BRK171"
slug: the-future-of-power-platform-intelligent-apps-brk171
conference: microsoft-ignite
conference_name: "Microsoft Ignite"
category: "Vendor events"
edition: "Ignite 2024"
year: 2024
speakers: ["Jonathan Beck", "Ryan Cunningham", "Clay Wesener"]
channel: "Microsoft Events"
duration_min: 49
published_at: 2024-11-21T15:10:38Z
video_id: Zzk5LEwy7w4
url: https://www.youtube.com/watch?v=Zzk5LEwy7w4
youtube_url: https://www.youtube.com/watch?v=Zzk5LEwy7w4
tags: ["BRK171", "Breakout", "Business Applications", "Business Applications-Curated", "Clay Wesener", "English (US)", "Intermediate (200)", "Jonathan Beck", "Ryan Cunningham", "Technical", "The Future of Power Platform: Intelligent Apps | BRK171", "Version v1", "ignite", "ignite 2024", "m6x0", "microsoft", "microsoft ignite", "microsoft ignite 2024", "ms ignite", "ms ignite 2024", "msft ignite", "msft ignite 2024"]
topics: []
transcript: false
---

# The Future of Power Platform: Intelligent Apps | BRK171

**Jonathan Beck, Ryan Cunningham, Clay Wesener**

`Microsoft Ignite` · `Ignite 2024` · `2024` · `49 min`

`#BRK171` `#Breakout` `#Business Applications` `#Business Applications-Curated` `#Clay Wesener` `#English (US)` `#Intermediate (200)` `#Jonathan Beck` `#Ryan Cunningham` `#Technical` `#The Future of Power Platform: Intelligent Apps | BRK171` `#Version v1` `#ignite` `#ignite 2024` `#m6x0` `#microsoft` `#microsoft ignite` `#microsoft ignite 2024` `#ms ignite` `#ms ignite 2024` `#msft ignite` `#msft ignite 2024`

[Watch the recording](https://www.youtube.com/watch?v=Zzk5LEwy7w4) · [Conference site](https://ignite.microsoft.com/)

## Description

Discover the future of business apps -- what they can do, how they're built, and how they're governed. We'll dive into the roadmap for Power Apps, Power Pages, and Managed Environments focusing on how Copilot is driving rapid evolution, all in this session packed with demos and customer success stories.

𝗦𝗽𝗲𝗮𝗸𝗲𝗿𝘀:
* Jonathan Beck
* Ryan Cunningham
* Clay Wesener

𝗦𝗲𝘀𝘀𝗶𝗼𝗻 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻:
This is one of many sessions from the Microsoft Ignite 2024 event. View even more sessions on-demand and learn about Microsoft Ignite at https://ignite.microsoft.com

BRK171 | English (US) | Business Applications

#MSIgnite
