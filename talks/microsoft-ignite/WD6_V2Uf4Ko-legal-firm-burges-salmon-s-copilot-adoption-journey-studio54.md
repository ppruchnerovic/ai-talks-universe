---
id: WD6_V2Uf4Ko
title: "Legal Firm Burges Salmon's Copilot Adoption Journey | Studio54"
slug: legal-firm-burges-salmon-s-copilot-adoption-journey-studio54
conference: microsoft-ignite
conference_name: "Microsoft Ignite"
category: "Vendor & platform"
edition: "Ignite 2024"
year: 2024
speakers: ["Chris Huntingford", "Alice Ratcliffe", "Dona Sarkar", "Eddie Twemlow"]
channel: "Microsoft Events"
duration_min: 10
published_at: 2024-11-25T22:58:09Z
video_id: WD6_V2Uf4Ko
url: https://www.youtube.com/watch?v=WD6_V2Uf4Ko
youtube_url: https://www.youtube.com/watch?v=WD6_V2Uf4Ko
tags: ["Alice Ratcliffe", "Chris Huntingford", "Dona Sarkar", "Eddie Twemlow", "English (US)", "Legal Firm Burges Salmon's Copilot Adoption Journey | Studio54", "Studio", "Studio54", "Version v0", "ignite", "ignite 2024", "m6x0", "microsoft", "microsoft ignite", "microsoft ignite 2024", "ms ignite", "ms ignite 2024", "msft ignite", "msft ignite 2024"]
transcript: false
---

# Legal Firm Burges Salmon's Copilot Adoption Journey | Studio54

**Chris Huntingford, Alice Ratcliffe, Dona Sarkar, Eddie Twemlow**

`Microsoft Ignite` · `Ignite 2024` · `2024` · `10 min`

`#Alice Ratcliffe` `#Chris Huntingford` `#Dona Sarkar` `#Eddie Twemlow` `#English (US)` `#Legal Firm Burges Salmon's Copilot Adoption Journey | Studio54` `#Studio` `#Studio54` `#Version v0` `#ignite` `#ignite 2024` `#m6x0` `#microsoft` `#microsoft ignite` `#microsoft ignite 2024` `#ms ignite` `#ms ignite 2024` `#msft ignite` `#msft ignite 2024`

[Watch the recording](https://www.youtube.com/watch?v=WD6_V2Uf4Ko) · [Conference site](https://ignite.microsoft.com/)

## Description

Are you thinking of deploying Microsoft 365 Copilot in your organization soon? If so, come and hear how UK-based legal firm Burges Salmon masterfully orchestrated their Copilot Deployment and Adoption this year. We will cover their journey from data security to Copilot adoption to Responsible AI to monitoring usage with the help of MS Partner and MVP Chris Huntingford.

𝗦𝗽𝗲𝗮𝗸𝗲𝗿𝘀:
* Chris Huntingford
* Alice Ratcliffe
* Dona Sarkar
* Eddie Twemlow

𝗦𝗲𝘀𝘀𝗶𝗼𝗻 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻:
This is one of many sessions from the Microsoft Ignite 2024 event. View even more sessions on-demand and learn about Microsoft Ignite at https://ignite.microsoft.com

Studio54 | English (US)

#MSIgnite
