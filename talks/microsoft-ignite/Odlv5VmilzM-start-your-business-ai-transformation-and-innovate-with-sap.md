---
id: Odlv5VmilzM
title: "Start your business AI transformation and innovate with SAP | BRKFP384"
slug: start-your-business-ai-transformation-and-innovate-with-sap
conference: microsoft-ignite
conference_name: "Microsoft Ignite"
category: "Vendor & platform"
edition: "Ignite 2024"
year: 2024
speakers: []
channel: "Microsoft Events"
duration_min: 47
published_at: 2024-11-25T15:29:04Z
video_id: Odlv5VmilzM
url: https://www.youtube.com/watch?v=Odlv5VmilzM
youtube_url: https://www.youtube.com/watch?v=Odlv5VmilzM
tags: ["AI", "BRKFP384", "Breakout", "Copilot for M365", "Diversity and Inclusion", "English (US)", "Enterprise work management", "Intermediate (200)", "Mike Graber", "Responsible AI", "Sivakumar N", "Start your business AI transformation and innovate with SAP | BRKFP384", "Sustainability", "Technical", "Workplace", "ignite", "ignite 2024", "m6x0", "microsoft", "microsoft ignite", "microsoft ignite 2024", "ms ignite", "ms ignite 2024", "msft ignite", "msft ignite 2024"]
transcript: false
---

# Start your business AI transformation and innovate with SAP | BRKFP384

**Speaker not identified**

`Microsoft Ignite` · `Ignite 2024` · `2024` · `47 min`

`#AI` `#BRKFP384` `#Breakout` `#Copilot for M365` `#Diversity and Inclusion` `#English (US)` `#Enterprise work management` `#Intermediate (200)` `#Mike Graber` `#Responsible AI` `#Sivakumar N` `#Start your business AI transformation and innovate with SAP | BRKFP384` `#Sustainability` `#Technical` `#Workplace` `#ignite` `#ignite 2024` `#m6x0` `#microsoft` `#microsoft ignite` `#microsoft ignite 2024` `#ms ignite` `#ms ignite 2024` `#msft ignite` `#msft ignite 2024`

[Watch the recording](https://www.youtube.com/watch?v=Odlv5VmilzM) · [Conference site](https://ignite.microsoft.com/)

## Description

Discover how SAP is harnessing the power of Business AI to drive meaningful transformation. This session will showcase the power of SAP BTP and how RISE with SAP on Microsoft Azure is a winning combination through demos, use-cases, and roadmaps. Start your AI-driven transformation with SAP and Microsoft.

𝗦𝗽𝗲𝗮𝗸𝗲𝗿𝘀:
* Mike Graber
* Sivakumar N

𝗦𝗲𝘀𝘀𝗶𝗼𝗻 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻:
This is one of many sessions from the Microsoft Ignite 2024 event. View even more sessions on-demand and learn about Microsoft Ignite at https://ignite.microsoft.com

BRKFP384 | English (US)

#MSIgnite
