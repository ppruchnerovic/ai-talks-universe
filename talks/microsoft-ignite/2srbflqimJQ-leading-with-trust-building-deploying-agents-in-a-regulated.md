---
id: 2srbflqimJQ
title: "Leading with Trust: Building & Deploying Agents in a Regulated World | BRK1733"
slug: leading-with-trust-building-deploying-agents-in-a-regulated
conference: microsoft-ignite
conference_name: "Microsoft Ignite"
category: "Vendor & platform"
edition: "Ignite 2025"
year: 2025
speakers: ["Sourabh Pandey", "Manny Sahota", "Tony Thampan"]
channel: "Microsoft Events"
duration_min: 44
published_at: 2025-11-24T13:52:17Z
video_id: 2srbflqimJQ
url: https://www.youtube.com/watch?v=2srbflqimJQ
youtube_url: https://www.youtube.com/watch?v=2srbflqimJQ
tags: ["9343d7d2-8cc4-410d-8335-a37de9eb4de3_Segment", "BRK1733", "English (US)", "Leading with Trust: Building & Deploying Agents in a Regulated World | BRK1733", "Manny Sahota", "Protect cloud AI platform and apps", "Sourabh Pandey", "Technical", "Tony Thampan", "f5n1", "ignite", "ignite 2025", "microsoft", "microsoft ignite", "microsoft ignite 2025", "ms ignite", "ms ignite 2025", "msft ignite", "msft ignite 2025"]
transcript: false
---

# Leading with Trust: Building & Deploying Agents in a Regulated World | BRK1733

**Sourabh Pandey, Manny Sahota, Tony Thampan**

`Microsoft Ignite` · `Ignite 2025` · `2025` · `44 min`

`#9343d7d2-8cc4-410d-8335-a37de9eb4de3_Segment` `#BRK1733` `#English (US)` `#Leading with Trust: Building & Deploying Agents in a Regulated World | BRK1733` `#Manny Sahota` `#Protect cloud AI platform and apps` `#Sourabh Pandey` `#Technical` `#Tony Thampan` `#f5n1` `#ignite` `#ignite 2025` `#microsoft` `#microsoft ignite` `#microsoft ignite 2025` `#ms ignite` `#ms ignite 2025` `#msft ignite` `#msft ignite 2025`

[Watch the recording](https://www.youtube.com/watch?v=2srbflqimJQ) · [Conference site](https://ignite.microsoft.com/)

## Description

AI and cyber regulations are accelerating worldwide. Learn how Microsoft Purview Compliance Manager, Regulatory Navigator, and Defender for Cloud align controls to laws like the EU AI Act, NIS2, DORA, and DPDP India; monitor posture in real time; and embed Responsible AI—turning regulatory resilience into cyber resilience and trusted innovation.

To learn more, please check out these resources:
* https://aka.ms/ignite25-plans-securedata

𝗦𝗽𝗲𝗮𝗸𝗲𝗿𝘀:
* Sourabh Pandey
* Manny Sahota
* Tony Thampan

𝗦𝗲𝘀𝘀𝗶𝗼𝗻 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻:
This is one of many sessions from the Microsoft Ignite 2025 event. View even more sessions on-demand and learn about Microsoft Ignite at https://ignite.microsoft.com

BRK1733 | English (US)

Breakout | Intermediate (200)

#MSIgnite, #ProtectcloudAIplatformandapps

Chapters:
0:00 - Growth of Agent Adoption and Compliance Importance
00:08:37 - Building Compliance into the Agent Lifecycle
00:16:12 - Automated compliance evaluations in Microsoft Purview for Foundry agents
00:18:19 - Implementing organization-wide communication compliance policies for real-time safety
00:20:29 - Continuous monitoring, maintenance, and demo of compliance lifecycle in action
00:24:32 - Launching AI-powered regulatory templates feature
00:30:09 - Creating an assessment to apply the new regulation and scope to Foundry projects
00:32:26 - Identifying and resolving failed high-risk actions through Foundry agent evaluation
00:35:39 - Post-Deployment Guardrails and Multi-Layer Security Analogy
