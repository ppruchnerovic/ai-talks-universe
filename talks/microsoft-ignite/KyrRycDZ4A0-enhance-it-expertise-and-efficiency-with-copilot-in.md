---
id: KyrRycDZ4A0
title: "Enhance IT expertise and efficiency with Copilot in Microsoft Intune | BRK320"
slug: enhance-it-expertise-and-efficiency-with-copilot-in
conference: microsoft-ignite
conference_name: "Microsoft Ignite"
category: "Vendor & platform"
edition: "Ignite 2024"
year: 2024
speakers: ["Carlos Brito", "Amit Ghodke", "Aaron Hamilton", "Archana Devi Sunder Rajan"]
channel: "Microsoft Events"
duration_min: 36
published_at: 2024-12-09T22:41:51Z
video_id: KyrRycDZ4A0
url: https://www.youtube.com/watch?v=KyrRycDZ4A0
youtube_url: https://www.youtube.com/watch?v=KyrRycDZ4A0
tags: ["Aaron Hamilton", "Advanced (300)", "Amit Ghodke", "Archana Devi Sunder Rajan", "BRK320", "Breakout", "Carlos Brito", "Copilot", "Data", "English (US)", "Enhance IT expertise and efficiency with Copilot in Microsoft Intune | BRK320", "GenerativeAI", "Microsoft Intune", "Security", "Security-Curated", "Technical", "ignite", "ignite 2024", "m6x0", "microsoft", "microsoft ignite", "microsoft ignite 2024", "ms ignite", "ms ignite 2024", "msft ignite", "msft ignite 2024"]
transcript: false
---

# Enhance IT expertise and efficiency with Copilot in Microsoft Intune | BRK320

**Carlos Brito, Amit Ghodke, Aaron Hamilton, Archana Devi Sunder Rajan**

`Microsoft Ignite` · `Ignite 2024` · `2024` · `36 min`

`#Aaron Hamilton` `#Advanced (300)` `#Amit Ghodke` `#Archana Devi Sunder Rajan` `#BRK320` `#Breakout` `#Carlos Brito` `#Copilot` `#Data` `#English (US)` `#Enhance IT expertise and efficiency with Copilot in Microsoft Intune | BRK320` `#GenerativeAI` `#Microsoft Intune` `#Security` `#Security-Curated` `#Technical` `#ignite` `#ignite 2024` `#m6x0` `#microsoft` `#microsoft ignite` `#microsoft ignite 2024` `#ms ignite` `#ms ignite 2024` `#msft ignite` `#msft ignite 2024`

[Watch the recording](https://www.youtube.com/watch?v=KyrRycDZ4A0) · [Conference site](https://ignite.microsoft.com/)

## Description

Explore how Copilot in Microsoft Intune assists admins in managing complex workflows with new policy management and troubleshooting generative AI functionality. Copilot provides deeper insights from your data, offers actionable guidance, and enhances IT expertise, leading to a more secure environment and efficient, informed decision-making.

To learn more, please check out these resources:
* https://aka.ms/TCL/MicroftSecurity
* https://learn.microsoft.com/en-us/security/
* https://aka.ms/IgniteAITools
* aka.ms/CopilotInIntune_Ignite_2024
* https://aka.ms/Ignite24Collection-IntuneandSecurity

𝗦𝗽𝗲𝗮𝗸𝗲𝗿𝘀:
* Carlos Brito
* Amit Ghodke
* Aaron Hamilton
* Archana Devi Sunder Rajan

𝗦𝗲𝘀𝘀𝗶𝗼𝗻 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻:
This is one of many sessions from the Microsoft Ignite 2024 event. View even more sessions on-demand and learn about Microsoft Ignite at https://ignite.microsoft.com

BRK320 | English (US) | Security

#MSIgnite
