---
id: AU5bzGQ_gEo
title: "Grow your business with Microsoft AI Cloud Partner Program | BRK337"
slug: grow-your-business-with-microsoft-ai-cloud-partner-program
conference: microsoft-ignite
conference_name: "Microsoft Ignite"
category: "Vendor events"
edition: "Ignite 2024"
year: 2024
speakers: ["Julie Sanford", "Steve Thomas"]
channel: "Microsoft Events"
duration_min: 34
published_at: 2024-11-22T08:32:16Z
video_id: AU5bzGQ_gEo
url: https://www.youtube.com/watch?v=AU5bzGQ_gEo
youtube_url: https://www.youtube.com/watch?v=AU5bzGQ_gEo
tags: ["BRK337", "Breakout", "English (US)", "Foundational (100)", "Grow your business with Microsoft AI Cloud Partner Program | BRK337", "Julie Sanford", "Partner Business", "Partner Program", "Steve Thomas", "Version v0", "ignite", "ignite 2024", "m6x0", "microsoft", "microsoft ignite", "microsoft ignite 2024", "ms ignite", "ms ignite 2024", "msft ignite", "msft ignite 2024"]
topics: []
transcript: false
---

# Grow your business with Microsoft AI Cloud Partner Program | BRK337

**Julie Sanford, Steve Thomas**

`Microsoft Ignite` · `Ignite 2024` · `2024` · `34 min`

`#BRK337` `#Breakout` `#English (US)` `#Foundational (100)` `#Grow your business with Microsoft AI Cloud Partner Program | BRK337` `#Julie Sanford` `#Partner Business` `#Partner Program` `#Steve Thomas` `#Version v0` `#ignite` `#ignite 2024` `#m6x0` `#microsoft` `#microsoft ignite` `#microsoft ignite 2024` `#ms ignite` `#ms ignite 2024` `#msft ignite` `#msft ignite 2024`

[Watch the recording](https://www.youtube.com/watch?v=AU5bzGQ_gEo) · [Conference site](https://ignite.microsoft.com/)

## Description

The Microsoft AI Cloud Partner Program unharnesses the power of what we can do when we deliver together. Join us to learn what's coming in the year ahead and how to create success in this rapidly expanding addressable market.

To learn more, please check out these resources:
* https://partner.microsoft.com/en-us/partnership
* https://learn.microsoft.com/en-us/partner-center/announcements/
* https://partner.microsoft.com/en-us/blog/article/innovate-with-partner-benefits
* https://partner.microsoft.com/blog/article/microsoft-partner-skilling
* https://partner.microsoft.com/connect/case-studies#/

𝗦𝗽𝗲𝗮𝗸𝗲𝗿𝘀:
* Julie Sanford
* Steve Thomas

𝗦𝗲𝘀𝘀𝗶𝗼𝗻 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻:
This is one of many sessions from the Microsoft Ignite 2024 event. View even more sessions on-demand and learn about Microsoft Ignite at https://ignite.microsoft.com

BRK337 | English (US) | Partner Program

#MSIgnite
