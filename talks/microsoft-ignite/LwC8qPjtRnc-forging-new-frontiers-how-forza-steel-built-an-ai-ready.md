---
id: LwC8qPjtRnc
title: "Forging new frontiers: How Forza Steel built an AI-ready foundation | ODSP1449"
slug: forging-new-frontiers-how-forza-steel-built-an-ai-ready
conference: microsoft-ignite
conference_name: "Microsoft Ignite"
category: "Vendor events"
edition: "Ignite 2025"
year: 2025
speakers: ["Juan Daniel Alonso", "Clay Caldwell"]
channel: "Microsoft Events"
duration_min: 22
published_at: 2025-11-19T14:27:54Z
video_id: LwC8qPjtRnc
url: https://www.youtube.com/watch?v=LwC8qPjtRnc
youtube_url: https://www.youtube.com/watch?v=LwC8qPjtRnc
tags: ["Clay Caldwell", "English (US)", "Forging new frontiers: How Forza Steel built an AI-ready foundation | ODSP1449", "Innovate with Azure AI apps and agents", "Juan Daniel Alonso", "Manufacturing and Mobility", "ODSP1449", "ODSP1449_v1", "Technical", "f5n1", "ignite", "ignite 2025", "microsoft", "microsoft ignite", "microsoft ignite 2025", "ms ignite", "ms ignite 2025", "msft ignite", "msft ignite 2025"]
topics: ["Enterprise adoption & strategy", "Science, healthcare & applied ML"]
transcript: false
---

# Forging new frontiers: How Forza Steel built an AI-ready foundation | ODSP1449

**Juan Daniel Alonso, Clay Caldwell**

`Microsoft Ignite` · `Ignite 2025` · `2025` · `22 min`

`#Clay Caldwell` `#English (US)` `#Forging new frontiers: How Forza Steel built an AI-ready foundation | ODSP1449` `#Innovate with Azure AI apps and agents` `#Juan Daniel Alonso` `#Manufacturing and Mobility` `#ODSP1449` `#ODSP1449_v1` `#Technical` `#f5n1` `#ignite` `#ignite 2025` `#microsoft` `#microsoft ignite` `#microsoft ignite 2025` `#ms ignite` `#ms ignite 2025` `#msft ignite` `#msft ignite 2025`

[Watch the recording](https://www.youtube.com/watch?v=LwC8qPjtRnc) · [Conference site](https://ignite.microsoft.com/)

## Description

Enterprise leaders are driving continuous transformation to stay at the forefront of real-time intelligent growth. Join us to learn how Forza Steel collaborated with Kyndryl and Microsoft to build a unified AI platform with Microsoft Fabric, Power BI, and Azure IoT to drive real-time insights, automation, and predictive analytics across manufacturing, logistics, and finance, as well as migrate their on-prem SAP to RISE on Azure.

To learn more, please check out these resources:
* https://www.kyndryl.com/industries/manufacturing/sap-on-azure-ebook/
* https://www.kyndryl.com/us/en/transform

𝗦𝗽𝗲𝗮𝗸𝗲𝗿𝘀:
* Juan Daniel Alonso
* Clay Caldwell

𝗦𝗲𝘀𝘀𝗶𝗼𝗻 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻:
This is one of many sessions from the Microsoft Ignite 2025 event. View even more sessions on-demand and learn about Microsoft Ignite at https://ignite.microsoft.com

ODSP1449 | English (US)

Pre-recorded | Intermediate (200)

#MSIgnite, #InnovatewithAzureAIappsandagents

Chapters:
0:00 - Clay Outlines Agenda: AI Platform, Microsoft Fabric, and SAP RISE Transition
00:05:30 - Focus on Scalability and Agility for Long-Term Growth
00:07:59 - Integration of Data Architecture Using Microsoft Azure and SAP
00:10:38 - Integration of Shop Floor Operations with CX Platform
00:12:34 - Clay Transitions to Generative AI Discussion
00:17:33 - Recognition of Data Cleansing as a Continuous, Never-Ending Journey
00:17:52 - Transition to Lessons Learned and Advice for IT Executives
