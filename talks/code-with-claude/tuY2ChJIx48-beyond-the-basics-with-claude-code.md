---
id: tuY2ChJIx48
title: "Beyond the basics with Claude Code"
slug: beyond-the-basics-with-claude-code
conference: code-with-claude
conference_name: "Code with Claude (Anthropic)"
category: "AI engineering & agents"
edition: "2026 London"
year: 2026
speakers: []
channel: "Claude"
duration_min: 47
published_at: 2026-05-22T14:29:22Z
video_id: tuY2ChJIx48
url: https://www.youtube.com/watch?v=tuY2ChJIx48
youtube_url: https://www.youtube.com/watch?v=tuY2ChJIx48
tags: []
transcript: false
---

# Beyond the basics with Claude Code

**Speaker not identified**

`Code with Claude (Anthropic)` · `2026 London` · `2026` · `47 min`

[Watch the recording](https://www.youtube.com/watch?v=tuY2ChJIx48) · [Conference site](https://claude.com/code-with-claude)

## Description

The mechanics that separate basic Claude Code use from real leverage: CLAUDE.md done well, wiring tools in with MCP, packaging team knowledge as skills, and using auto mode safely.
