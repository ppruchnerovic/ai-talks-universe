---
id: oDks2gVHu4k
title: "Building Blocks for Tomorrow’s AI Agents"
slug: building-blocks-for-tomorrows-ai-agents
conference: code-with-claude
conference_name: "Code with Claude (Anthropic)"
category: "AI engineering & agents"
edition: "2025"
year: 2025
speakers: []
channel: "Anthropic"
duration_min: 29
published_at: null
video_id: oDks2gVHu4k
url: https://www.youtube.com/watch?v=oDks2gVHu4k
youtube_url: https://www.youtube.com/watch?v=oDks2gVHu4k
tags: []
transcript: false
---

# Building Blocks for Tomorrow’s AI Agents

**Speaker not identified**

`Code with Claude (Anthropic)` · `2025` · `2025` · `29 min`

[Watch the recording](https://www.youtube.com/watch?v=oDks2gVHu4k) · [Conference site](https://claude.com/code-with-claude)

## Description

*No description published on YouTube.*
