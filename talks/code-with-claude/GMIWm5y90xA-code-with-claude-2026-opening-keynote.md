---
id: GMIWm5y90xA
title: "Code with Claude 2026: Opening Keynote"
slug: code-with-claude-2026-opening-keynote
conference: code-with-claude
conference_name: "Code with Claude (Anthropic)"
category: "Practitioner AI conferences"
edition: "2026 San Francisco"
year: 2026
speakers: []
channel: "Claude"
duration_min: 48
published_at: 2026-05-06T16:58:56Z
video_id: GMIWm5y90xA
url: https://www.youtube.com/watch?v=GMIWm5y90xA
youtube_url: https://www.youtube.com/watch?v=GMIWm5y90xA
tags: []
topics: []
transcript: false
---

# Code with Claude 2026: Opening Keynote

**Speaker not identified**

`Code with Claude (Anthropic)` · `2026 San Francisco` · `2026` · `48 min`

[Watch the recording](https://www.youtube.com/watch?v=GMIWm5y90xA) · [Conference site](https://claude.com/code-with-claude)

## Description

Get the latest updates from Anthropic's engineering and product leaders at the Code with Claude 2026 opening keynote in San Francisco.
