---
id: -OAebRdfUhI
title: "[Keynote] MCP Project Update with David Soria Parra - Anthropic"
slug: keynote-mcp-project-update-with-david-soria-parra-anthropic
conference: mcp-dev-summit
conference_name: "MCP Dev Summit"
category: "AI engineering & agents"
edition: "MCP Dev Summit Europe 2025"
year: 2025
speakers: []
channel: "Agentic AI Foundation"
duration_min: 28
published_at: 2025-10-09T13:44:41Z
video_id: -OAebRdfUhI
url: https://www.youtube.com/watch?v=-OAebRdfUhI
youtube_url: https://www.youtube.com/watch?v=-OAebRdfUhI
tags: ["mcp", "model context protocol", "mcp dev summit", "mcp summit", "mcp ai"]
topics: ["Agents & orchestration"]
transcript: false
---

# [Keynote] MCP Project Update with David Soria Parra - Anthropic

**Speaker not identified**

`MCP Dev Summit` · `MCP Dev Summit Europe 2025` · `2025` · `28 min`

`#mcp` `#model context protocol` `#mcp dev summit` `#mcp summit` `#mcp ai`

[Watch the recording](https://www.youtube.com/watch?v=-OAebRdfUhI) · [Conference site](https://events.linuxfoundation.org/mcp-dev-summit-north-america/)

## Description

MCP Project Update
🎤 David Soria Parra, Member of Technical Staff - Anthropic

✨ MCP Developers Summit EU 2025
📅 2 Oct 2025
📍 London, UK
