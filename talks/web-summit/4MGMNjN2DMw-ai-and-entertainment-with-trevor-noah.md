---
id: 4MGMNjN2DMw
title: "AI and entertainment with Trevor Noah"
slug: ai-and-entertainment-with-trevor-noah
conference: web-summit
conference_name: "Web Summit"
category: "Industry & business"
edition: "Web Summit"
year: 2024
speakers: []
channel: "Web Summit"
duration_min: 36
published_at: 2024-08-16T08:00:03Z
video_id: 4MGMNjN2DMw
url: https://www.youtube.com/watch?v=4MGMNjN2DMw
youtube_url: https://www.youtube.com/watch?v=4MGMNjN2DMw
tags: ["web summit", "web summit conference lisbon", "web summit paddy", "web summit youtube", "web summit lisboa", "Lisbon", "Lisboa"]
transcript: false
---

# AI and entertainment with Trevor Noah

**Speaker not identified**

`Web Summit` · `Web Summit` · `2024` · `36 min`

`#web summit` `#web summit conference lisbon` `#web summit paddy` `#web summit youtube` `#web summit lisboa` `#Lisbon` `#Lisboa`

[Watch the recording](https://www.youtube.com/watch?v=4MGMNjN2DMw) · [Conference site](https://websummit.com)

## Description

Wish you were here? https://websummit.com/tickets?utm_source=youtube&utm_medium=video&utm_campaign=WS_Video
