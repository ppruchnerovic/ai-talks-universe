---
id: NsX8Uge_7T0
title: "Media literacy in the age of AI"
slug: media-literacy-in-the-age-of-ai
conference: web-summit
conference_name: "Web Summit"
category: "Industry & business"
edition: "Web Summit"
year: 2024
speakers: []
channel: "Web Summit"
duration_min: 25
published_at: 2024-03-21T12:09:41Z
video_id: NsX8Uge_7T0
youtube_url: https://www.youtube.com/watch?v=NsX8Uge_7T0
tags: ["web summit", "web summit conference lisbon", "web summit paddy", "web summit youtube", "web summit lisboa", "Lisbon", "Lisboa"]
transcript: false
---

# Media literacy in the age of AI

**Speaker not identified**

`Web Summit` · `Web Summit` · `2024` · `25 min`

`#web summit` `#web summit conference lisbon` `#web summit paddy` `#web summit youtube` `#web summit lisboa` `#Lisbon` `#Lisboa`

[Watch the recording](https://www.youtube.com/watch?v=NsX8Uge_7T0) · [Conference site](https://websummit.com)

## Description

Wish you were here? https://websummit.com/tickets?utm_source=youtube&utm_medium=video&utm_campaign=WS_Video
